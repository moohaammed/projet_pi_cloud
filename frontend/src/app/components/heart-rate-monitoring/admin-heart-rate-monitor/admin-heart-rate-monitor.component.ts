...
import { HeartRateLiveState, HeartRateRecord, HeartRateService } from '../../../services/heart-rate.service';
import { HeartRateLiveStateService } from '../../../services/heart-rate-live-state.service';

@Component({
  selector: 'app-admin-heart-rate-monitor',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './admin-heart-rate-monitor.component.html',
  styleUrl: '../shared-heart-rate-monitor.css'
})
export class AdminHeartRateMonitorComponent implements OnInit, OnDestroy {
  patients: MonitoredPatient[] = [];
  states = new Map<number, HeartRateLiveState>();
  histories = new Map<number, HeartRateRecord[]>();
  ...
  loadPatients(): void {
    ...
    if (!user?.id) {
      this.errorMessage = 'Unable to identify the connected admin.';
      return;
    }

    this.isLoading = true;
    this.errorMessage = '';
    this.accessService.getAdminPatients(user.id).subscribe({
      next: patients => {
      ...
      error: () => {
        this.isLoading = false;
        this.errorMessage = 'Unable to load patients for admin monitoring.';
      }
    });
  }
  ...
  get attentionCount(): number {
    return this.patients.filter(patient => {
      ...
      return this.isConnected(patient) && bpm != null && (bpm < 60 || bpm > 100);
    }).length;
  }
  ...
  toggleHistory(patient: MonitoredPatient): void {
    if (typeof patient.userId !== 'number') {
      return;
    }

    if (this.expandedUserId === patient.userId) {
      this.expandedUserId = null;
      return;
    }

    this.expandedUserId = patient.userId;
    if (this.histories.has(patient.userId)) {
      return;
    }

    this.loadingHistory.add(patient.userId);
    this.heartRateService.getHistory(patient.userId).subscribe({
      next: records => {
        this.histories.set(patient.userId as number, (records || []).slice(0, 50));
        this.loadingHistory.delete(patient.userId as number);
        this.cdr.detectChanges();
      },
      error: () => {
        this.histories.set(patient.userId as number, []);
        this.loadingHistory.delete(patient.userId as number);
        this.cdr.detectChanges();
      }
    });
  ...
  isHistoryOpen(patient: MonitoredPatient): boolean {
    return typeof patient.userId === 'number' && this.expandedUserId === patient.userId;
  }

  isHistoryLoading(patient: MonitoredPatient): boolean {
    return typeof patient.userId === 'number' && this.loadingHistory.has(patient.userId);
  }

  getHistory(patient: MonitoredPatient): HeartRateRecord[] {
    return typeof patient.userId === 'number' ? this.histories.get(patient.userId) || [] : [];
  }

  formatRecordTime(record: HeartRateRecord): string {
    return this.formatTimestamp(record.receivedAt || record.recordedAt || record.capturedAt || '');
  }

  getRecordZone(record: HeartRateRecord): string {
    return this.liveStateService.formatZone(undefined, record.bpm);
  }

  getRecordClass(record: HeartRateRecord): string {
    if (record.bpm < 60) {
      return 'low';
    }
    if (record.bpm > 100) {
  ...
  private loadStatesAndStream(): void {
    ...
    this.streamSubscription = this.heartRateService.connectLiveStream(userIds).subscribe({
      next: event => {
        this.ngZone.run(() => {
          const state = this.liveStateService.toLiveState(event);
          this.states.set(event.userId, state);
          this.prependExpandedHistory(event);
          this.cdr.detectChanges();
        });
      },
      error: () => {
  ...
  private prependExpandedHistory(event: HeartRateRecord): void {
    if (this.expandedUserId !== event.userId || !this.histories.has(event.userId)) {
      return;
    }
    const existing = this.histories.get(event.userId) || [];
    this.histories.set(event.userId, [
      { ...event, recordedAt: event.receivedAt || event.recordedAt || '' },
      ...existing
    ].slice(0, 50));
  }
