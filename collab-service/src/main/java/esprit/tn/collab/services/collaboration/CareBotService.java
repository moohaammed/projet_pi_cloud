...
public class CareBotService {
    ...
    public boolean sendMedicationReminderToPatient(Long userId) {
        ...
        sendMedicationRemindersTo(List.of(user));
        return true;
    }
    ...
    public boolean handleMedicationAcknowledgment(Long userId, boolean tookMedication) {
        ...
        // Persist the compliance record for analytics
    ...
    public void processMessageForSupport(Message userMsg) {
        ...
        if (userClient.isRole(sender, "PATIENT") && userMsg.getContent() != null) {
            ...
            // Check 1: disorientation phrases
            if (DISORIENTATION_PATTERN.matcher(content).find()) {
                handleDisorientation(userMsg, sender);
                return; // don't run other checks after disorientation
            }

            // Check 2: repetitive questions — same message 3+ times in 30 minutes
            Instant thirtyMinsAgo = Instant.now().minusSeconds(1800);
            List<Message> recent = messageRepository.findBySenderIdAndSentAtAfterOrderBySentAtDesc(
                    userMsg.getSenderId(), thirtyMinsAgo);
            if (recent.size() >= 3) {
                long count = recent.stream().limit(3)
                        .filter(m -> m.getContent() != null && m.getContent().trim().equalsIgnoreCase(content))
                        .count();
                if (count >= 3) {
                    logSafetyAlert(userMsg.getSenderId(), SafetyAlertType.REPETITIVE_QUESTIONS,
                            SafetyAlertStatus.OPEN, userMsg.getId());
                }
            }
        }

        // Check 3: high distress score (applies to all senders, not just patients)
        if (userMsg.getSentimentScore() != null && userMsg.getSentimentScore() <= -0.2
                && userMsg.getSenderId() != null) {
            Map<String, Object> sender2 = userClient.getUserById(userMsg.getSenderId());
            if (userClient.isRole(sender2, "PATIENT")) {
                logSafetyAlert(userMsg.getSenderId(), SafetyAlertType.HIGH_DISTRESS_SIGNAL,
                        SafetyAlertStatus.OPEN, userMsg.getId());
            }
    ...
    private void handleDisorientation(Message userMsg, Map<String, Object> patient) {
        ...
        // Log the alert
        logSafetyAlert(patientId, SafetyAlertType.DISORIENTATION,
                SafetyAlertStatus.CAREGIVERS_NOTIFIED, userMsg.getId());

        // Reassure the patient
        sendBotPrivateMessage(patientId,
                "It's okay, everything is fine. You are at home. I have notified your family. ❤️");
    ...
    private void logSafetyAlert(Long patientId, SafetyAlertType type,
                                 SafetyAlertStatus status, String relatedMessageId) {
    ...
    private void sendBotPrivateMessage(Long targetUserId, String content, MessageType type) {
        ...
        botMsg.setSenderId(null); // null senderId = bot message
    ...
    public void sendReassurance(Long targetUserId) {
        ...
        sendBotPrivateMessage(targetUserId,
                "Hello! I noticed you might be going through a tough moment. Remember that you're not alone! ❤️");
    }
    ...
    public void injectMemoryAnchor() {
        userClient.getUsersByRole("PATIENT").forEach(patient -> {
            Long patientId = ((Number) patient.get("id")).longValue();
            notificationService.createAndSend(patientId,
                    "CareBot: Time for a memory! Check the feed for a special post.", "MEMORY_ANCHOR");
        });
    }
}
